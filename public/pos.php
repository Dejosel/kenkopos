...
<body class="pos-body">
    ...
    <header class="pos-header">
        <div class="d-flex align-items-center gap-3">
            <div class="pos-logo">
                <img src="assets/img/logo_kenkopos.png" alt="KenkoPOS" height="75" style="object-fit:contain; vertical-align:middle;">
            </div>
        ...
        <div class="d-flex align-items-center gap-3">
            ...
            <div class="bg-dark border border-secondary rounded px-3 py-1 d-flex align-items-center gap-2" id="header-operator-badge" style="cursor: pointer;" onclick="logoutOperator()" title="Haga clic para cerrar sesión">
                <i class="bi bi-person-circle text-warning fs-5"></i>
                <div class="text-start">
                    <div class="text-white fw-bold lh-1 small" id="header-operator-name">Cargando...</div>
                    <span class="badge bg-secondary p-1 lh-1 text-uppercase" style="font-size: 0.65rem;" id="header-operator-role">...</span>
                </div>
                <i class="bi bi-box-arrow-right text-danger ms-2" style="font-size: 1.1rem;"></i>
            </div>
            
            <a href="reports.php" class="btn btn-outline-warning btn-sm px-3" id="btn-reports-access">
                <i class="bi bi-bar-chart-fill me-1"></i> Reportes
            </a>
            <a href="products/list.php" class="btn btn-outline-light btn-sm px-3" id="btn-catalog-access">
    ...
    <!-- PANTALLA COMPLETA DE LOGIN (Oculta el POS completamente) -->
    <div id="login-screen" class="d-none align-items-center justify-content-center" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: linear-gradient(135deg, #090d16 0%, #1e293b 100%); z-index: 99999; overflow-y: auto;">
        <div class="card border-0 shadow-lg p-4 mx-3" style="width: 100%; max-width: 450px; background: rgba(30, 41, 59, 0.75); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.08); border-radius: 16px;">
            ...
            <div id="login-error-alert" class="alert alert-danger d-none py-2 px-3 mb-3 text-center" style="font-size: 0.9rem;">
                Credenciales incorrectas
            </div>
            
            <!-- Operadores Rápidos (SambaPOS Style) -->
            <p class="text-white-50 small mb-3">Selecciona tu rol de operador rápido:</p>
            <div class="row row-cols-3 g-2 mb-4 justify-content-center">
                <div class="col">
                    <button type="button" class="btn btn-outline-warning w-100 py-3 d-flex flex-column align-items-center gap-1 border-secondary" onclick="selectQuickOperator('admin@kenkopos.com')">
                        <i class="bi bi-person-badge-fill fs-3 text-warning"></i>
                        <span class="fw-semibold small text-white">Admin</span>
                        <code class="text-white-50" style="font-size: 0.65rem;">admin123</code>
                    </button>
                </div>
                <div class="col">
                    <button type="button" class="btn btn-outline-info w-100 py-3 d-flex flex-column align-items-center gap-1 border-secondary" onclick="selectQuickOperator('cajero@kenkopos.com')">
                        <i class="bi bi-cash-stack fs-3 text-info"></i>
                        <span class="fw-semibold small text-white">Cajero</span>
                        <code class="text-white-50" style="font-size: 0.65rem;">cajero123</code>
                    </button>
                </div>
                <div class="col">
                    <button type="button" class="btn btn-outline-success w-100 py-3 d-flex flex-column align-items-center gap-1 border-secondary" onclick="selectQuickOperator('mesero@kenkopos.com')">
                        <i class="bi bi-person-fill fs-3 text-success"></i>
                        <span class="fw-semibold small text-white">Mesero</span>
                        <code class="text-white-50" style="font-size: 0.65rem;">mesero123</code>
                    </button>
                </div>
            </div>

            <div class="d-flex align-items-center my-3 text-white-50">
                <hr class="flex-grow-1 border-secondary m-0">
                <span class="mx-2 small text-uppercase" style="font-size: 0.7rem;">O iniciar con correo</span>
                <hr class="flex-grow-1 border-secondary m-0">
            </div>

            <form id="pos-login-form" onsubmit="submitPosLogin(event)">
                <div class="mb-3 text-start">
                    <label for="login-email" class="form-label text-white-50 small mb-1">Correo Electrónico</label>
                    <div class="input-group">
                        <span class="input-group-text bg-dark border-secondary text-white-50"><i class="bi bi-envelope"></i></span>
                        <input type="email" class="form-control bg-dark border-secondary text-white" id="login-email" required placeholder="correo@kenkopos.com">
                    </div>
                </div>
                <div class="mb-4 text-start">
                    <label for="login-password" class="form-label text-white-50 small mb-1">Contraseña</label>
                    <div class="input-group">
                        <span class="input-group-text bg-dark border-secondary text-white-50"><i class="bi bi-lock"></i></span>
                        <input type="password" class="form-control bg-dark border-secondary text-white" id="login-password" required placeholder="••••••••">
                    </div>
                </div>
                <button type="submit" class="btn btn-primary w-100 py-2.5 fw-bold fs-5 shadow" id="btn-login-submit">
